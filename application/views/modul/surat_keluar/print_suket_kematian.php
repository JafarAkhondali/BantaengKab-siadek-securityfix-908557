...
foreach ($cetak as $row) {
  ...
  $date_kematian = date_create($row['waktu_kematian']);
  $day = date_format($date, "d");
  $bln = date_format($date, "m");
  $thn = date_format($date, "Y");
  $tanggal_kematian = date_format($date_kematian,  "d - m - Y");
  $haris_kematian = date_format($date_kematian,  "N");
  $jam_kematian = date_format($date_kematian,  "H:i");


  switch ($haris_kematian) {
    case '1':
      $hari_kematian = "SENIN";
      break;
    case '2':
      $hari_kematian = "SELASA";
      break;
    case '3':
      $hari_kematian = "RABU";
      break;
  ...
  $html = '<h4 align="center"><U>SURAT KETERANGAN KEMATIAN</U> <br> Nomor: ' . $row['no'] . '</h4>
          ...
          <p>Benar yang namanya tersebut diatas adalah Warga di ' . strtolower($klasifikasik) . ' ' . $desa . ' Kecamatan ' . $camat . ' Kabupaten Bantaeng Yang telah Meninggal dunia pada : </p> 
          
          <table cellspacing="17">
            <tr>
              <td>Hari/Tanggal Kematian</td>
              <td colspan="3">: ' . $hari_kematian . '/'. $tanggal_kematian .'</td>
            </tr>
            <tr>
            <td>Pukul</td>
            <td colspan="3">: ' . $jam_kematian . '</td>
            </tr>
            <tr>
            <td>Tempat Kematian</td>
            <td colspan="3">: ' . $tempat_kematian . '</td><br>
            </tr>
            
            
          </table>
          
          <p><br><br>Demikian surat keterangan ini diberikan untuk dipergunakan sebagaimana mestinya.<br><br>
          </p>
         <table>
...
$pdf->Output('Surat Keterangan Kematian.pdf', 'I');
?>
