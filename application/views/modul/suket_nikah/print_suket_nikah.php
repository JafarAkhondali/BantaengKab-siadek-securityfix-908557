...
$pemohons = db_get_all_data('penduduk', "nik = $nik");
...
$istris = db_get_all_data('penduduk', "nik = $nik_is");
foreach ($pemohons as $row) {
  $nama_istri = $row->nama;
  $alamat_istri = $row->alamat;
  $tempat_lahir_istri = $row->tempat_lahir;
  $date_lahir_istri = date_create($row->tgl_lahir);
  $tanggal_lahir_istri = date_format($date_lahir_istri, "d - m - Y");
  $jen_peker_istri = $row->jenis_pekerjaan;
  $jenis_kelamin_istri = $row->jenis_kelamin;
  $agama_istri = $row->agama;
}
...
$pekerjaans_istri = db_get_all_data('setup_pekerjaan', "id_pekerjaan = $jen_peker_istri");
foreach ($pekerjaans_istri as $value) {
  $pekerjaan_istri = $value->value;
}
...
foreach ($cetak as $row) {
  ...
  $html = '<h4 align="center"><U>SURAT KETERANGAN MENIKAH</U> <br> Nomor: ' . $row['no'] . '</h4>
            ...
            <tr>
              <td>Nama Isteri</td>
              <td colspan="3">: ' . $nama_istri . '</td>
            </tr>
            <tr>
            <td>Tempat/Tgl Lahir</td>
            <td colspan="3">: ' . $tempat_lahir_istri . '/' . $tanggal_lahir_istri . '</td>
            </tr>
            <tr>
            <td>Jenis Kelamin</td>
            <td colspan="3">: ' . $jenis_kelamin_istri . '</td>
            </tr>
            <tr>
            <td>Agama</td>
            <td colspan="3">: ' . $agama_istri . '</td>
            </tr>
            <tr>
            <td>Pekerjaan</td>
            <td colspan="3">: ' . $pekerjaan_istri . '</td>
            </tr>
            <tr>
            <td>Alamat</td>
            <td colspan="3">: ' . $alamat_istri . '</td>
            </tr>
            
            
          </table>

          <p>Benar yang namanya tersebut diatas adalah Warga di ' . strtolower($klasifikasik) . ' ' . $desa . ' Kecamatan ' . $camat . ' Kabupaten Bantaeng <b>Dan telah menjadi pasangan suami isteri</b> .<br><br>Demikian surat keterangan ini diberikan untuk dipergunakan sebagaimana mestinya.<br><br>
          </p>
         <table>
...
$pdf->Output('Surat Keterangan Menikah.pdf', 'I');
?>
