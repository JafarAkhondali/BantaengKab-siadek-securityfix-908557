                ...
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                        ...
                        <div class="widget-user-header ">
                            ...
                            <!-- /.widget-user-image -->
                            <h3 class="widget-user-username"><b>Suket Belum Menikah</b></h3>
                            <h5 class="widget-user-desc">Edit Suket Belum Menikah</h5>
                            <hr>
                        </div>
                        <?= form_open(base_url('suket_bnikah/edit_save/'.$this->uri->segment(3)), [
                            'name'    => 'form_suket_bnikah', 
                            'class'   => 'form-horizontal', 
                            'id'      => 'form_suket_bnikah', 
                            'method'  => 'POST'
                            ]); ?>
              ...
              <div class="col-sm-8">
                ...
                <small class="info help-block">
                  <b>Input Kd Wilayah</b> Max Length : 30.</small>
                            ...
                            <div class="col-sm-8">
                                <input type="text" class="form-control" name="no" id="no" placeholder="NO. Surat" value="<?= set_value('no', $suket_bnikah->no); ?>">
                                <small class="info help-block">
                            ...
                            <div class="input-group date col-sm-8">
                              <input type="text" class="form-control pull-right datepicker" name="tanggal_surat"  placeholder="Tanggal Surat" id="tanggal_surat" value="<?= set_value('suket_bnikah_tanggal_surat_name', $suket_bnikah->tanggal_surat); ?>">
                            </div>
                            <small class="info help-block">
                            </small>
                            </div>
              ...
              <div class="col-sm-8">
                ...
                <select class="form-control chosen chosen-select-deselect" name="nik" id="nik" data-placeholder="Select Nik">
                  ...
                  <?php foreach ($a as $row) : ?>
                    ...
                    <option <?php if ($row->nik == $suket_bnikah->nik) { ?>selected="selected" <?php } ?> value="<?= $row->nik ?>"><?= '[ ' . $row->nik . ' ] ' . $row->nama ?></option>
                  <?php endforeach; ?>
                </select>
                <small class="info help-block">
                </small>
    ...
    $(document).ready(function(){
      ...
      $('#btn_cancel').click(function(){
        swal({
          ...
          function(isConfirm){
            if (isConfirm) {
              window.location.href = BASE_URL + 'suket_bnikah';
            }
          });
      ...
      $('.btn_save').click(function(){
        ...
        var form_suket_bnikah = $('#form_suket_bnikah');
        var data_post = form_suket_bnikah.serializeArray();
        ...
        $.ajax({
          url: form_suket_bnikah.attr('action'),
          type: 'POST',
          dataType: 'json',
          data: data_post,
        })
        .done(function(res) {
          if(res.success) {
            var id = $('#suket_bnikah_image_galery').find('li').attr('qq-file-id');
            if (save_type == 'back') {
