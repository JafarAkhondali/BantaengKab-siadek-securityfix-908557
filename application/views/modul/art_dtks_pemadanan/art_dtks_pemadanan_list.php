...
function domo(){
   ...
   $('*').bind('keydown', 'Ctrl+a', function assets() {
       window.location.href = BASE_URL + '/Art_dtks_pemadanan/add';
       return false;
   });
            ...
            <div class="box-body ">
               ...
               <div class="box box-widget widget-user-2">
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        <?php is_allowed('art_dtks_pemadanan_add', function(){?>
                        <a class="btn btn-flat btn-success btn_add_new" id="btn_add_new" title="Tambah Data  (Ctrl+a)" href="<?=  site_url('art_dtks_pemadanan/add'); ?>"><i class="fa fa-plus-square-o" ></i> Tambah Data</a>
                        <?php }) ?>
                        <?php is_allowed('art_dtks_pemadanan_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> XLS" href="<?= site_url('art_dtks_pemadanan/export'); ?>"><i class="fa fa-file-excel-o" ></i></a>
                        <?php }) ?>
                        <?php is_allowed('art_dtks_pemadanan_export', function(){?>
                        <a class="btn btn-flat btn-success" title="<?= cclang('export'); ?> PDF" href="<?= site_url('art_dtks_pemadanan/export_pdf'); ?>"><i class="fa fa-file-pdf-o" ></i></a>
                        <?php }) ?>
                     </div>
                     ...
                     <h5 class="widget-user-desc"><?= cclang('list_all', ['Art Dtks Pemadanan']); ?>  <i class="label bg-yellow"><?= $art_dtks_pemadanan_counts; ?>  <?= cclang('items'); ?></i></h5>
                  </div>
                  
                   

                  <form name="form_art_dtks_pemadanan" id="form_art_dtks_pemadanan" action="<?= base_url('art_dtks_pemadanan/index'); ?>">
                  <br>
                  <br>
                    ...
                    <div class="form-group ">
                            ...
                            <div class="col-sm-3">
                                <select  class="form-control chosen chosen-select-deselect" name="Status" id="Status" data-placeholder="PILIH Status" onchange="submit()">
                                  <option value="0"></option>
                                    <option 
                                    <?php if ('1' == $this->input->get('Status')) { ?>selected="selected"<?php } ?>
                                    value="1">Sesuai</option>
                                     <option 
                                    <?php if ('2' == $this->input->get('Status')) { ?>selected="selected"<?php } ?>
                                    value="2">Nik tidak Sesuai</option>
                                     <option 
                                    <?php if ('3' == $this->input->get('Status')) { ?>selected="selected"<?php } ?>
                                    value="3">Sudah Meninggal</option>
                                     <option 
                                    <?php if('4' == $this->input->get('Status')) { ?>selected="selected"<?php } ?>
                                    value="4">Wilayah Tidak Sesuai</option>
                    ...
                    <table class="table table-bordered table-striped dataTable" >
                     <thead>
                        <tr class="">
                           ...
                           <th style="text-align:center" width='10%'>Sta Keberadaan Art</th>
                           <th style="text-align:center" width='10%'>Sta Kepesertaan Pbi</th>
                     ...
                     <?php foreach($art_dtks_pemadanans as $art_dtks_pemadanan): ?>
                        <tr>
                           <td width="5">
                              <input type="checkbox" class="flat-red check" name="id[]" value="<?= $art_dtks_pemadanan->id_art_dtks_pemadanan; ?>">
                           </td>
                           
                           <td><?= _ent($art_dtks_pemadanan->Nama); ?></td> 
                           <td><?= _ent($art_dtks_pemadanan->NIK); ?></td> 
                           <td style="text-align:center"><? if($art_dtks_pemadanan->Sta_keberadaan_art ==1){echo '<button class="border-0 btn-transition bg-olive  btn btn-outline-success">
                                                                <i class="fa fa-check">';}else{echo '<button class="border-0 btn-transition btn-danger  btn btn-outline-success">
                                                                <i class=" fa fa-remove">';}; ?>
                           </td> 
                           <td style="text-align:center"><?php if($art_dtks_pemadanan->Sta_kepesertaan_pbi==1){echo '<button class="border-0 btn-transition bg-olive  btn btn-outline-success">
                                                                <i class="fa fa-check">';}else {echo '<button class="border-0 btn-transition btn-danger  btn btn-outline-success">
                                                                <i class=" fa fa-remove">';}; ?></td> 
                           <td><?php if($art_dtks_pemadanan->Ada_kks==1){echo '<button class="border-0 btn-transition bg-olive  btn btn-outline-success">
                                                                <i class="fa fa-check">';}else {echo '<button class="border-0 btn-transition btn-danger  btn btn-outline-success">
                                                                <i class=" fa fa-remove">';}; ?></td> 
                           <td><?php if($art_dtks_pemadanan->Ada_pbi==1){echo '<button class="border-0 btn-transition bg-olive  btn btn-outline-success">
                                                                <i class="fa fa-check">';}else {echo '<button class="border-0 btn-transition btn-danger  btn btn-outline-success">
                                                                <i class=" fa fa-remove">';}; ?></td> 
                           <td><?php if($art_dtks_pemadanan->Ada_kip==1){echo '<button class="border-0 btn-transition bg-olive  btn btn-outline-success">
                                                                <i class="fa fa-check">';}else{echo '<button class="border-0 btn-transition btn-danger  btn btn-outline-success">
                                                                <i class=" fa fa-remove">';}; ?></td> 
                           <td><?php if($art_dtks_pemadanan->Ada_pkh==1){echo '<button class="border-0 btn-transition bg-olive  btn btn-outline-success">
                                                                <i class="fa fa-check">';}else{echo '<button class="border-0 btn-transition btn-danger  btn btn-outline-success">
                                                                <i class=" fa fa-remove">';}; ?></td> 
                           <td>
                            <?php if($art_dtks_pemadanan->Ada_BPNT==1){echo '<button class="border-0 btn-transition bg-olive  btn btn-outline-success">
                                                                <i class="fa fa-check">';}else{echo '<button class="border-0 btn-transition btn-danger  btn btn-outline-success">
                                                                <i class=" fa fa-remove">';}; ?>
                            </td>
                           ...
                           <td width="200">
                              <?php is_allowed('art_dtks_pemadanan_view', function() use ($art_dtks_pemadanan){?>
                              <a href="<?= site_url('art_dtks_pemadanan/view/' . $art_dtks_pemadanan->id_art_dtks_pemadanan); ?>" title="Lihat" class="label-default"><i class="fa fa-newspaper-o"></i> 
                              <?php }) ?>
                              <?php is_allowed('art_dtks_pemadanan_update', function() use ($art_dtks_pemadanan){?>
                              <a href="<?= site_url('art_dtks_pemadanan/edit/' . $art_dtks_pemadanan->id_art_dtks_pemadanan); ?>" title="Ubah" class="label-default"><i class="fa fa-edit "></i> </a>
                              <?php }) ?>
                              <?php is_allowed('art_dtks_pemadanan_delete', function() use ($art_dtks_pemadanan){?>
                              <a href="javascript:void(0);" data-href="<?= site_url('art_dtks_pemadanan/delete/' . $art_dtks_pemadanan->id_art_dtks_pemadanan); ?>" title="Hapus" class="label-default remove-data"><i class="fa fa-close" style="color: red"></i> </a>
                               <?php }) ?>
                           </td>
                        </tr>
                      <?php endforeach; ?>
                      <?php if ($art_dtks_pemadanan_counts == 0) :?>
                         <tr>
                           <td colspan="100">
                           Art Dtks Pemadanan data is not available
                           </td>
                         </tr>
                      <?php endif; ?>
                     </tbody>
               ...
               <div class="row">
                  <div class="col-md-8">
                     ...
                     <div class="col-sm-1 padd-left-0 ">
                        <a class="btn btn-default btn-flat" name="reset" id="reset" value="Apply" href="<?= base_url('art_dtks_pemadanan');?>" title="<?= cclang('reset_filter'); ?>">
...
<script>
  $(document).ready(function(){
    ...
    $('#apply').click(function(){
      ...
      var bulk = $('#bulk');
      var serialize_bulk = $('#form_art_dtks_pemadanan').serialize();
      ...
      if (bulk.val() == 'delete') {
         swal({
          ...
          function(isConfirm){
            if (isConfirm) {
               document.location.href = BASE_URL + '/art_dtks_pemadanan/delete?' + serialize_bulk;      
            }
          });
