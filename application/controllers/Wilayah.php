...
class Wilayah extends Admin	
{
	...
	*/
	public function index($offset = 0)
	{
		...
		$this->render('modul/wilayah/wilayah_list', $this->data);
	}
	...
	*/
	public function add()
	{
		...
		$this->render('modul/wilayah/wilayah_add', $this->data);
	}
	...
	*/
	public function add_save()
	{
		...
		$this->form_validation->set_rules('kd_wilayah', 'Kd Wilayah', 'trim|required|max_length[50]');
		$this->form_validation->set_rules('nama', 'Nama', 'trim|required|max_length[30]');
		$this->form_validation->set_rules('klasifikasi', 'Klasifikasi', 'trim|required|max_length[30]');
		$this->form_validation->set_rules('kd_induk', 'Kd Induk', 'trim|max_length[50]');
		...
		if ($this->form_validation->run()) {
			...
			$save_data = [
				...
				'kd_induk' => $this->input->post('kd_induk'),
			];
			...
			if ($save_wilayah) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('wilayah/edit/' . $save_wilayah, 'Edit Wilayah'),
						anchor('wilayah', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('wilayah');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
	...
	*/
	public function edit($id)
	{
		...
		$this->render('modul/wilayah/wilayah_update', $this->data);
	}
	...
	*/
	public function view($id)
	{
		...
		$this->data['wilayah'] = $this->model_wilayah->join_avaiable()->filter_avaiable()->find($id);

		$this->template->title('Wilayah Detail');
		$this->render('modul/wilayah/wilayah_view', $this->data);
	}
