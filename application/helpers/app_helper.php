...
if(!function_exists('db_get_wilayah_like')) {
	function db_get_wilayah_like($where = null) {
		$ci =& get_instance();
		if ($where) {
			$ci->db->like('kd_wilayah',$where);
		}
	  	$query = $ci->db->get('wilayah');

	    return $query->result();
	}
}

if(!function_exists('db_get_all_data_count')) {
	function db_get_all_data_count($table_name = null, $where = false) {
		$ci =& get_instance();
		if ($where) {
			$ci->db->where($where);
		}
...
if(!function_exists('get_list_surat')) {
	function get_list_surat($table_name = null) {
		$ci =& get_instance();
		
	  	$query = $ci->db->query('SELECT no,tanggal_surat,id FROM '.$table_name.'');

	    return $query->result();
	}
}

if(!function_exists('setup_get_data')) {
	function setup_get_data($table_name = null, $value = null) {
		$ci =& get_instance();
		$ci->db->where('value',$value);
	  	$query = $ci->db->get($table_name);
	  	$row = $query->row();
...
if(!function_exists('setup_get_bps')) {
	function setup_get_bps($value = null) {
		$ci =& get_instance();
		$ci->db->where('kd_wilayah',$value);
	  	$query = $ci->db->get('wilayah');
	  	$row = $query->row();
	 
       return $row->kd_wilayah_bps;

	   
	}
}

if(!function_exists('setup_get_regulasi')) {
	function setup_get_regulasi( $value = null) {
		$ci =& get_instance();
		$ci->db->where('regulasi_jns_kode',$value);
	  	$query = $ci->db->get('tbl_regulasi_jenis');
	  	$row = $query->row();
	 
       return $row->regulasi_jns_nama;
...
if(!function_exists('setup_get_wilayah')) {
	function setup_get_wilayah($table_name = null, $value = null) {
		$ci =& get_instance();
		$ci->db->where('namab',$value);
		
	  	$query = $ci->db->get($table_name);
	  	$row = $query->row();
	 
       return $row->kd_wilayah;

	   
	}
}

if(!function_exists('setup_get_token')) {
	function setup_get_token($value = null) {
		$ci =& get_instance();
		$ci->db->where('site',$value);
	  	$query = $ci->db->get('token');
	  	$row = $query->row();
...
if(!function_exists('get_user_group')) {
	function get_user_group($id = null) {
		...
		$ci->db->where('user_id',$id);
	  	$query = $ci->db->get('aauth_user_to_group');
